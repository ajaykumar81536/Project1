...
void Clean_nd_Validate(const char* input_str) {
    ...
    // Function to check if one VALID_Stringg is smaller than the other
    bool operator<(const VALID_Stringg& other) const {
        int i = 0;
        while (String_data[i] != '\0' && other.String_data[i] != '\0') {
         ...
         // If all characters matched so far, but one string is shorter
        return (String_data[i] == '\0' && other.String_data[i] != '\0');
    }
